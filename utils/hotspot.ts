...
export function snapToBoxCenterOrInside(point: Point, box?: Box | null): Point {
  if (!box) return clampPoint(point);
  const center = {
...
export function ensureMinDistance(
  normalizedPoints: Point[],
  minDistancePx: number,
  container: Size,
  maxIterations = 16,
): Point[] {
  if (normalizedPoints.length <= 1 || minDistancePx <= 0) return normalizedPoints.map(clampPoint);
...
const FALLBACK_DIRECTIONS: Array<{ x: number; y: number }> = [
  { x: 1, y: 0 },
  { x: -1, y: 0 },
  { x: 0, y: 1 },
  { x: 0, y: -1 },
...
function clampWithPadding(value: number, padding: number): number {
...
function sanitizePoint(point?: Partial<Point> | null): Point {
  if (!point || typeof point !== "object") return { x: 0.5, y: 0.5 };
...
function sanitizeBox(raw?: Partial<Box> | null): Box | null {
  if (!raw || typeof raw !== "object") return null;
  let x = Number((raw as any).x ?? (raw as any).left ?? 0);
  let y = Number((raw as any).y ?? (raw as any).top ?? 0);
  let w = Number((raw as any).w ?? (raw as any).width ?? 0);
  let h = Number((raw as any).h ?? (raw as any).height ?? 0);
  ...
  if (x + w > 1) x = clamp01(1 - w);
  if (y + h > 1) y = clamp01(1 - h);
...
export interface HotspotLayoutInput {
  point?: Partial<Point> | null;
  box?: Partial<Box> | null;
}

export interface HotspotLayoutResult {
  points: Point[];
  boxes: Array<Box | null>;
}

export function normalizeHotspotLayout(
  inputs: HotspotLayoutInput[],
  options: { padding?: number; minDistance?: number } = {},
): HotspotLayoutResult {
