...
async function createSignature(secretKey, message) {
    if (typeof globalThis !== "undefined" && globalThis?.crypto?.subtle) {
        const subtle = globalThis.crypto.subtle;
        const encoder = new TextEncoder();
        const key = await subtle.importKey("raw", encoder.encode(secretKey), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
...
async function buildSignature(env, opts, timestamp) {
...
function isValidProduct(raw, tokens = []) {
    const link = raw?.productUrl ?? raw?.productUrlMobile ?? "";
    const image = raw?.productImage ?? raw?.productImageMobile ?? "";
    ...
    const containsSoldOut = SOLDOUT_KEYWORDS.some((kw) => textBlob.includes(kw));
    if (containsSoldOut)
        return false;
    if (tokens.length && !includesAllTokens(name, tokens))
        return false;
    return true;
}
async function searchProducts(query, env) {
    ...
    if (env.partnerId)
        params.set("subId", env.partnerId);
    // eslint-disable-next-line no-console
    console.log(`[COUPANG_API] search="${query.query}" tokens=${(query.mustTokens ?? []).join(",")}`);
    const raw = await coupangFetch({
        method: "GET",
        path: SEARCH_PATH,
        query: params,
    });
...
async function buildDeeplink(url, env) {
    if (!url)
        return url;
    ...
    const deeplink = response?.data?.[0]?.shortenUrl ?? response?.data?.[0]?.url ?? url;
    // eslint-disable-next-line no-console
